...
import { AccNotificationsMenu } from "@/components/acc/acc-notifications-menu";
import { useDismiss } from "@/hooks/use-dismiss";
import { ACC_BASE, accNavLabelFor } from "@/lib/acc/nav";
import { accInitial, useAccProfile } from "@/lib/acc/profile-store";

const MENU_ITEM =
  "flex items-center gap-3 px-5 py-3 text-[15px] transition-colors hover:bg-gray-50";

export function AccTopbar({ onOpenMenu }: { onOpenMenu: () => void }) {
      ...
      <div className="flex min-w-0 items-center gap-3">
        ...
        <nav aria-label="Breadcrumb" className="min-w-0">
          <ol className="flex items-center gap-2 text-[13px]">
            <li className="font-semibold text-[#1b3a5c]">Accountant</li>
            <li aria-hidden="true" className="text-gray-300">
              ›
            </li>
            <li aria-current="page" className="truncate text-muted">
              {accNavLabelFor(pathname)}
            </li>
          </ol>
        </nav>
      </div>
