...
import { SO_BASE, soNavGroups, type SoNavItem } from "@/lib/so/nav";

const ROW =
  "flex w-full items-center gap-3 rounded-md px-3 py-2.5 text-[14px] transition-colors";
...
function NavBody({
  ...
  // Sections start closed; one opens as soon as you are inside it.
    ...
    <nav className="flex-1 overflow-y-auto p-3">
      <ul className="space-y-0.5">
        {soNavGroups.map((group, index) => {
          ...
          // A collapsed rail has no room for section headings — the child icons
          // stand in for them.
            ...
            <li key={group.title ?? index}>
              {!collapsed && (
                <button
                  ...
                  className={`${ROW} ${ROW_IDLE} text-[12px] font-semibold tracking-wide text-gray-500 uppercase focus-visible:ring-2 focus-visible:ring-brand/30 focus-visible:outline-none`}
                >
                  <Icon
                    aria-hidden="true"
    ...
    <>
      ...
      >
        ...
        <div className="flex h-[76px] items-center border-b border-hairline px-4">
          <Link href={`${SO_BASE}/dashboard`} aria-label="Azentra dashboard">
            <Logo
              markOnly={collapsed}
