...
import { PmNotificationsMenu } from "@/components/pm/pm-notifications-menu";
import { useDismiss } from "@/hooks/use-dismiss";
import { PM_BASE, pmNavLabelFor } from "@/lib/pm/nav";
import { pmInitial, usePmProfile } from "@/lib/pm/profile-store";

const MENU_ITEM =
  "flex items-center gap-3 px-5 py-3 text-[15px] transition-colors hover:bg-gray-50";

export function PmTopbar({ onOpenMenu }: { onOpenMenu: () => void }) {
      ...
      <div className="flex min-w-0 items-center gap-3">
        ...
        <nav aria-label="Breadcrumb" className="min-w-0">
          <ol className="flex items-center gap-2 text-[13px]">
            <li className="font-semibold text-[#1b3a5c]">Property Manager</li>
            <li aria-hidden="true" className="text-gray-300">
              ›
            </li>
            <li aria-current="page" className="truncate text-muted">
              {pmNavLabelFor(pathname)}
            </li>
          </ol>
        </nav>
      </div>
