...
import { FS_BASE, fsNavGroups, type FsNavItem } from "@/lib/fs/nav";

const ROW =
  "flex w-full items-center gap-3 rounded-md px-3 py-2.5 text-[13px] transition-colors";
            ...
            <li key={group.title ?? index}>
              {!collapsed && (
                <button
                  ...
                  className={`${ROW} ${ROW_IDLE} focus-visible:ring-2 focus-visible:ring-brand/30 focus-visible:outline-none`}
                >
                  <Icon aria-hidden="true" className="h-4 w-4 shrink-0" />
    ...
    <>
      ...
      >
        ...
        <div className="flex h-[76px] items-center border-b border-hairline px-4">
          <Link href={`${FS_BASE}/dashboard`} aria-label="Azentra dashboard">
            <Logo
              markOnly={collapsed}
